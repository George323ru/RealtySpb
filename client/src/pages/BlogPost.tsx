...
const heatingArticles: BlogPost[] = [
  {
    ...
    content: `\nМиф 1: Это незаконно.\nНа самом деле: Установка автономного отопления регулируется законодательством, и при соблюдении всех норм и получении разрешений это абсолютно законно. Главное — правильно оформленный проект и согласование с управляющей компанией и соответствующими инстанциями.\n\nМиф 2: Это очень дорого.\nНа самом деле: Первоначальные вложения действительно существенны, но в долгосрочной перспективе вы экономите до 50-70% на коммунальных платежах за отопление. Срок окупаемости системы обычно составляет от 3 до 5 лет.\n\nМиф 3: Это опасно.\nНа самом деле: Современные газовые и электрические котлы оснащены многоуровневыми системами безопасности, включая датчики утечки газа, контроль тяги и автоматическое отключение при перегреве. При профессиональном монтаже и регулярном обслуживании риск сведен к минимуму.\n\nМиф 4: Я буду зависеть от электричества/газа.\nНа самом деле: Вы и так от них зависите. Центральное отопление тоже может отключаться. Преимущество автономии в том, что вы сами контролируете температуру и включаете отопление только тогда, когда это нужно, а не по графику города.\n\nМиф 5: Это сложно в управлении.\nНа самом деле: Современные системы управляются интуитивно понятными термостатами, в том числе и через смартфон. Вы можете задать нужную температуру, настроить расписание работы и забыть о ручной регулировке.\n    `,
  ...
  },
  {
    ...
    content: `\nВыбор между газовым и электрическим котлом — одно из ключевых решений при проектировании системы отопления.\n\nГазовый котел:\nПлюсы:\n- Низкая стоимость эксплуатации: Газ пока остается самым дешевым видом топлива.\n- Высокая мощность: Легко отапливают большие площади.\n- Долговечность: Срок службы газовых котлов — 15-20 лет.\nМинусы:\n- Сложность установки: Требуется проект, согласования, дымоход.\n- Высокая стоимость оборудования и монтажа.\n- Подключение к газовой магистрали возможно не везде.\n\nЭлектрический котел:\nПлюсы:\n- Простота установки: Не нужен дымоход, минимум согласований.\n- Низкая стоимость оборудования.\n- Компактность и бесшумность.\n- Экологичность: Нет продуктов сгорания.\nМинусы:\n- Высокая стоимость эксплуатации: Электричество значительно дороже газа.\n- Требуется качественная проводка и достаточная выделенная мощность.\n\nВывод: Если у вас есть доступ к магистральному газу и вы готовы к первоначальным вложениям, газовый котел — ваш выбор для экономии в будущем. Если газа нет, или площадь дома небольшая, электрический котел станет простым и быстрым решением.\n    `,
    author: 'Иван Сергеев',
    category: 'Инженерные системы',
  ...
  },
  {
    ...
    content: `\nТеплый пол — это комфорт, который вы ощущаете буквально кожей. Но ошибки при монтаже могут свести все плюсы на нет.\n\nКлючевые моменты:\n\n1. Правильная подготовка основания: Поверхность должна быть идеально ровной и чистой. Обязательна укладка теплоизоляции (например, пеноплекса), чтобы не греть плиту перекрытия или подвал.\n\n2. Выбор системы:\n- Водяной теплый пол: Идеален для частных домов. Требует правильной укладки труб (улитка или змейка) и подключения к коллектору.\n- Электрический (кабельный): Подходит для квартир. Важно правильно рассчитать мощность на м² и шаг укладки кабеля.\n- Пленочный (инфракрасный): Лучший вариант под ламинат или линолеум. Монтируется "сухим" способом.\n\n3. Проверка перед заливкой стяжки: Обязательно проведите гидравлические испытания для водяного пола или замер сопротивления для электрического. Зафиксируйте результаты. Это ваша страховка.\n\n4. Демпферная лента: Обязательно проложите ее по периметру помещения. Она компенсирует тепловое расширение стяжки и предотвратит ее растрескивание.\n\n5. Первый запуск: Не включайте систему на полную мощность сразу после монтажа. Стяжка должна набрать прочность (около 28 дней). Повышайте температуру постепенно.\n    `,
...
export default function BlogPostPage() {
  ...
  const { data: blogPost, isLoading, error } = useQuery<BlogPost>({
    ...
    queryFn: async () => {
      const mockArticle = heatingArticles.find(a => a.slug === slug);
      if (mockArticle) {
